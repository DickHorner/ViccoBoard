...
| Totals calculated correctly | `VERIFIED` | `modules/exams/src/use-cases/record-correction.use-case-v2.ts:52-57` — `totalPoints` summed from taskScores; grade computed via `GradingKeyService.calculateGrade`; tests: "calculates grade correctly from grading key", "calculates totals from multiple task scores" |
