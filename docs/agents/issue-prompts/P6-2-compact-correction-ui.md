...
## Scope Lock
- Primary issue: `P6-2` - Compact Correction UI
...
## Traceability
- Relates to: Plan.md section 6.11 (Kompakte Korrekturmaske)

## Evidence Required (mandatory)
- Create or update `docs/evidence/P6-2-evidence.md` from `docs/evidence/_template.md`.
...
1. Resolve the GitHub issue number for `P6-2` before coding.
   - Run: `gh issue list --state open --search "P6-2 in:title"`
2. Sync from `main` and branch only from `origin/main`.
   - Run in order:
     - `git fetch origin`
     - `git checkout main`
     - `git pull --ff-only origin main`
     - `git checkout -b copilot/p6-2-<short-slug>`
...
6. Commit with a traceable message that includes `P6-2`.
7. Push branch and open a PR to `main`.
   - PR title starts with `[P6-2] ...`
   - PR body must include: `Closes #<issue_number>` (primary issue only)
8. After merge, verify the linked issue is actually closed.
   - If not closed, run: `gh issue close <issue_number> --comment "Closed via merged PR for P6-2."`
...
1. Pre-edit declaration:
   ...
   - risks/dependencies
   - GitHub issue number resolved for `P6-2`
   - branch name to use
