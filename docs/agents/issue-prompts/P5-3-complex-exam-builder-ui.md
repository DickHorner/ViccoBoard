...
1. Resolve the GitHub issue number for `P5-3` before coding.
   - Run: `gh issue list --state open --search "P5-3 in:title"`
2. Sync from `main` and branch only from `origin/main`.
   - Run in order:
     - `git fetch origin`
     - `git checkout main`
     - `git pull --ff-only origin main`
     - `git checkout -b copilot/p5-3-<short-slug>`
...
6. Commit with a traceable message that includes `P5-3`.
7. Push branch and open a PR to `main`.
   - PR title starts with `[P5-3] ...`
   - PR body must include: `Closes #<issue_number>` (primary issue only)
8. After merge, verify the linked issue is actually closed.
   - If not closed, run: `gh issue close <issue_number> --comment "Closed via merged PR for P5-3."`
...
1. Pre-edit declaration:
   ...
   - risks/dependencies
   - GitHub issue number resolved for `P5-3`
   - branch name to use
