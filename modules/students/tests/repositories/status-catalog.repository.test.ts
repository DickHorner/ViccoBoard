...
describe('StatusCatalogRepository', () => {
  ...
  let repo: StatusCatalogRepository;
  ...
  describe('getOrCreateForClassGroup', () => {
    ...
    it('should create separate catalogs for different contexts', async () => {
      const attendance = await repo.getOrCreateForClassGroup(
        'class-1',
        'attendance'
      );
      const participation = await repo.getOrCreateForClassGroup(
        'class-1',
        'participation'
      );
  ...
  describe('updateStatus', () => {
    ...
    it('should throw error if status not found', async () => {
      ...
      await expect(
        repo.updateStatus(catalog.id, 'non-existent', { name: 'Test' })
      ).rejects.toThrow('not found');
    });
  });
  ...
  describe('findByClassGroupAndContext', () => {
    ...
    it('should return null if not found', async () => {
      const found = await repo.findByClassGroupAndContext(
        'non-existent',
        'attendance'
      );
