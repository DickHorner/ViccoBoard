...
describe('TimeGradingService', () => {
    ...
    describe('createDefaultConfig', () => {
        ...
        test('throws error if best time is greater than or equal to worst time', () => {
            expect(() => service.createDefaultConfig(120, 60, 1, 6)).toThrow('Best time must be less than worst time');
            expect(() => service.createDefaultConfig(100, 100, 1, 6)).toThrow('Best time must be less than worst time');
        });
    });
    describe('validateConfig', () => {
        ...
        test('throws error for non-linear mapping', () => {
            ...
            expect(() => service.validateConfig(config)).toThrow('Only linear mapping is currently supported');
        });
    ...
    describe('calculateGrade', () => {
        ...
        describe('error cases', () => {
            test('throws error for non-linear mapping', () => {
                ...
                expect(() => service.calculateGrade({ timeInSeconds: 90, config })).toThrow('Only linear mapping is currently supported');
            });
            test('throws error for missing boundaries', () => {
                ...
                expect(() => service.calculateGrade({ timeInSeconds: 90, config })).toThrow('Custom boundaries with time values are required for time-based grading');
            });
            test('throws error for insufficient boundaries', () => {
                ...
                expect(() => service.calculateGrade({ timeInSeconds: 90, config })).toThrow('At least 2 boundaries (best and worst) are required');
            });
        });
    });
    ...
    describe('sample data scenarios', () => {
        ...
        test('post-hoc adjustment after grading', () => {
            ...
            const adjustedGrade = service.calculateGrade({ timeInSeconds: 90, config }).grade;
            // 90 is now 33.3% of the way from 70 to 130
            // Grade = 1 + 0.333 * 5 = 2.67 (approximately)
            expect(adjustedGrade).toBeCloseTo(2.67, 1);
            expect(adjustedGrade).toBeLessThan(initialGrade1);
        });
    });
});
