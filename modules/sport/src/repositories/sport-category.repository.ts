/**
 * Sport Category Repository
 * Stores raw Sport category schema fields in grade_categories.
 */
...
export class SportCategoryRepository {
  ...
  async save(entity: SportSchema.SportCategory): Promise<SportSchema.SportCategory> {
  ...
  async findById(id: string): Promise<SportSchema.SportCategory | null> {
    const row = await this.adapter.getById(this.tableName, id);
    return row ? this.mapToEntity(row) : null;
  }

  async findAll(): Promise<SportSchema.SportCategory[]> {
  ...
  private mapToEntity(row: Record<string, unknown>): SportSchema.SportCategory {
    return {
      ...
      categories: safeJsonParse(row.categories as string | undefined, undefined, 'SportCategory.categories'),
      ...
      settings: safeJsonParse(row.settings as string | undefined, undefined, 'SportCategory.settings'),
      stats: safeJsonParse(row.stats as string | undefined, undefined, 'SportCategory.stats'),
  ...
  private mapToRow(entity: SportSchema.SportCategory): Record<string, unknown> {
    ...
    return {
      ...
      year: entity.year,
      categories: entity.categories ? safeJsonStringify(entity.categories, 'SportCategory.categories') : undefined,
      color: entity.color,
      ...
      configuration: entity.settings ? safeJsonStringify(entity.settings, 'SportCategory.configuration') : '{}',
      deleted: entity.deleted,
      is_dirty: entity.is_dirty,
      main_category_id: entity.main_category_id,
      max_value: entity.max_value,
      min_value: entity.min_value,
      settings: entity.settings ? safeJsonStringify(entity.settings, 'SportCategory.settings') : undefined,
      stats: entity.stats ? safeJsonStringify(entity.stats, 'SportCategory.stats') : undefined,
