 ...
 * 
 * @remarks
 * - Pure calculation engine with no network dependencies (offline-first compliant)
 * - No browser-specific APIs used (Safari/WebKit compatible)
 * - All data structures are plain TypeScript interfaces
 */
...
export class CriteriaGradingEngine {
   ...
   * 
   * Note: Criterion minValue/maxValue are metadata for UI purposes.
   * All score values must be normalized to 0-100 scale before calculation.
   */
   ...
   * 
   * Grade boundaries are inclusive on lower bound, exclusive on upper bound.
   * The order of evaluation ensures proper boundary handling:
   * - Grade 1: 92% ≤ score ≤ 100%
   * - Grade 2: 81% ≤ score < 92%
   * - Grade 3: 67% ≤ score < 81%
   * - Grade 4: 50% ≤ score < 67%
   * - Grade 5: 30% ≤ score < 50%
  ...
  updateWeights(criteria: Criterion[], newWeights: Map<string, number>): Criterion[] {
    ...
    // Apply updates to get final criteria - always return new objects for consistency
    const updatedCriteria = criteria.map(criterion => {
      const newWeight = newWeights.get(criterion.id);
      return newWeight !== undefined 
        ? { ...criterion, weight: newWeight }
        : { ...criterion }; // Return copy for consistency
    });
