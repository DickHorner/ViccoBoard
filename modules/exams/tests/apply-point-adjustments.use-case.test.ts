...
describe('ApplyPointAdjustmentsUseCase Integration Tests (#326)', () => {
  ...
  it('applies reviewed point adjustments and persists recalculated corrections', async () => {
    const exam = await examRepo.create({
      ...
      structure: {
        parts: [],
        tasks: [
          createTask('task-a', 'Analysis', 20, 1),
          createTask('task-b', 'Transfer', 30, 2)
        ],
        allowsComments: true,
      ...
      taskScores: [
        {
          ...
          points: 10,
          maxPoints: 20,
          criterionScores: [{ criterionId: 'criterion-a', points: 4, maxPoints: 8 }],
          timestamp: new Date()
        },
        {
          taskId: 'task-b',
    ...
    await applyPointAdjustmentsUseCase.execute({
      examId: exam.id,
      adjustments: [
        { taskId: 'task-a', suggestedPoints: 10 },
        { taskId: 'task-b', suggestedPoints: 40 }
      ]
    });
    ...
    expect(reloadedExam?.structure.tasks.find((task) => task.id === 'task-a')?.points).toBe(10);
    expect(reloadedExam?.structure.tasks.find((task) => task.id === 'task-b')?.points).toBe(40);
    ...
    expect(taskAScore?.points).toBe(5);
    expect(taskAScore?.maxPoints).toBe(10);
    expect(taskAScore?.criterionScores?.[0]).toEqual({
      criterionId: 'criterion-a',
      points: 2,
      maxPoints: 4
    });
    expect(taskBScore?.points).toBe(20);
    expect(taskBScore?.maxPoints).toBe(40);
    expect(reloadedCorrection?.totalPoints).toBe(25);
    expect(reloadedCorrection?.percentageScore).toBe(50);
    expect(reloadedCorrection?.totalGrade).toBe('4');
  });
