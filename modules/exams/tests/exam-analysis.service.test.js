 ...
 */
import { ExamAnalysisService, AnalysisUIHelper } from '../src/services/exam-analysis.service';
...
describe('ExamAnalysisService', () => {
    ...
    describe('suggestPointAdjustments', () => {
        it('should return adjustment suggestions', () => {
            const result = ExamAnalysisService.suggestPointAdjustments(mockExam, mockCorrections, 0.6);
        ...
        it('should maintain total points across adjustments', () => {
            const result = ExamAnalysisService.suggestPointAdjustments(mockExam, mockCorrections, 0.6);
            const currentTotal = Object.values(result.currentDistribution).reduce((a, b) => a + b, 0);
            const suggestedTotal = Object.values(result.suggestedDistribution).reduce((a, b) => a + b, 0);
    ...
    describe('identifyStudentsAtRisk', () => {
        ...
        it('should calculate critical threshold as 70% of target', () => {
            ...
            const critical = result.filter((r) => r.riskLevel === 'critical');
            const warning = result.filter((r) => r.riskLevel === 'warning');
