...
import { AlternativeGradingService, AlternativeGradingUIHelper, STANDARD_ALTERNATIVE_SCALE, SIMPLIFIED_ALTERNATIVE_SCALE } from '../src/services/alternative-grading.service';
describe('AlternativeGradingService', () => {
    describe('toNumericPoints', () => {
        ...
        it('should throw error for invalid grade type', () => {
            expect(() => {
                AlternativeGradingService.toNumericPoints('invalid', 100, STANDARD_ALTERNATIVE_SCALE);
            }).toThrow();
        });
    ...
    describe('getGradeConfig', () => {
        ...
        it('should throw error for invalid grade', () => {
            expect(() => {
                AlternativeGradingService.getGradeConfig('invalid', STANDARD_ALTERNATIVE_SCALE);
            }).toThrow();
        });
    });
    describe('createAlternativeGrading', () => {
        ...
        it('should set correct points for each grade', () => {
            const grades = ['++', '+', '0', '-', '--'];
            const maxPoints = 100;
            const results = grades.map(grade => AlternativeGradingService.createAlternativeGrading(grade, maxPoints, STANDARD_ALTERNATIVE_SCALE));
