...
describe('SupportTipManagementService', () => {
    ...
    let testCorrection;
    beforeEach(() => {
        testTip = SupportTipManagementService.createSupportTip('Decimal places', 'Practice working with decimal numbers', {
            category: 'Math',
    ...
    describe('createSupportTip', () => {
        it('should create a support tip', () => {
            const tip = SupportTipManagementService.createSupportTip('Test Tip', 'Test description');
        ...
        it('should create with optional parameters', () => {
            const tip = SupportTipManagementService.createSupportTip('Advanced Tip', 'Description', {
                category: 'Physics',
                tags: ['forces'],
                priority: 2,
                weight: 8
    ...
    describe('addLink', () => {
        it('should add link to support tip', () => {
            const updated = SupportTipManagementService.addLink(testTip, 'Khan Academy', 'https://www.khanacademy.org/math/decimals');
        ...
        it('should not allow more than 3 links', () => {
            let updated = testTip;
            for (let i = 0; i < 3; i++) {
                updated = SupportTipManagementService.addLink(updated, `Link ${i + 1}`, `https://example.com/${i}`);
            }
            expect(updated.links.length).toBe(3);
            expect(() => {
                SupportTipManagementService.addLink(updated, 'Link 4', 'https://example.com/4');
            }).toThrow();
        });
    });
    describe('removeLink', () => {
        it('should remove link from support tip', () => {
            let updated = SupportTipManagementService.addLink(testTip, 'Test Link', 'https://example.com');
    ...
    describe('searchSupportTips', () => {
        ...
        beforeEach(() => {
            tips = [
                SupportTipManagementService.createSupportTip('Decimals Basic', 'Learn decimal basics', { category: 'Math', tags: ['basic', 'decimals'] }),
                SupportTipManagementService.createSupportTip('Fractions Advanced', 'Advanced fraction techniques', { category: 'Math', tags: ['advanced', 'fractions'] }),
                SupportTipManagementService.createSupportTip('Article Writing', 'Technical article writing guide', { category: 'English', tags: ['writing', 'articles'] })
            ];
        });
        ...
        it('should filter by category', () => {
            const results = SupportTipManagementService.searchSupportTips(tips, '', { category: 'English' });
            expect(results.length).toBe(1);
        ...
        it('should sort by usage', () => {
            ...
            tips[1].usageCount = 5;
            tips[2].usageCount = 0;
            const results = SupportTipManagementService.searchSupportTips(tips, '', { sortBy: 'usage' });
            expect(results[0].usageCount).toBeGreaterThanOrEqual(results[1].usageCount);
        });
        it('should limit results', () => {
            const results = SupportTipManagementService.searchSupportTips(tips, '', { limit: 2 });
    ...
    describe('assignTipToCorrection', () => {
        it('should assign tip to correction', () => {
            const updated = SupportTipManagementService.assignTipToCorrection(testCorrection, testTip.id);
        ...
        it('should not assign same tip twice', () => {
            let updated = SupportTipManagementService.assignTipToCorrection(testCorrection, testTip.id);
            updated = SupportTipManagementService.assignTipToCorrection(updated, testTip.id);
            expect(updated.supportTips.length).toBe(1);
        });
        it('should allow task-specific assignment', () => {
            const updated = SupportTipManagementService.assignTipToCorrection(testCorrection, testTip.id, 'task-1');
    ...
    describe('removeTipFromCorrection', () => {
        it('should remove tip from correction', () => {
            let updated = SupportTipManagementService.assignTipToCorrection(testCorrection, testTip.id);
            updated = SupportTipManagementService.removeTipFromCorrection(updated, testTip.id);
            expect(updated.supportTips.length).toBe(0);
        });
    });
    describe('getTipsForCorrection', () => {
        it('should return assigned tips', () => {
            const correction = SupportTipManagementService.assignTipToCorrection(testCorrection, testTip.id);
            const result = SupportTipManagementService.getTipsForCorrection([testTip], correction);
        ...
        it('should filter by task', () => {
            ...
            let correction = SupportTipManagementService.assignTipToCorrection(testCorrection, testTip.id, 'task-1');
            correction = SupportTipManagementService.assignTipToCorrection(correction, tip2.id, undefined);
            const taskSpecific = SupportTipManagementService.getTipsForCorrection([testTip, tip2], correction, 'task-1');
    ...
    describe('getSummaryForExam', () => {
        it('should calculate usage summary', () => {
            ...
            const summary = SupportTipManagementService.getSummaryForExam([testTip, tip2], corrections);
...
describe('SupportTipUIHelper', () => {
    let testTip;
    beforeEach(() => {
        testTip = SupportTipManagementService.createSupportTip('Test Tip', 'Test description', { category: 'Math', weight: 5 });
    });
