 ...
 */
import { normalizePaginationOptions, createPaginatedResult, safeJsonParse, safeJsonStringify } from '@viccoboard/core';
import { db } from '../db';
/**
 * Map database record to CorrectionEntry domain entity
 */
const mapRecordToCorrection = (record) => ({
    id: record.id,
 ...
 */
const mapCorrectionToRecord = (entry) => ({
    id: entry.id,
    examId: entry.examId,
...
export function useCorrections() {
    const getByExam = async (examId, pagination) => {
        try {
    ...
    const getByCandidate = async (candidateId, pagination) => {
        try {
            const options = normalizePaginationOptions(pagination);
