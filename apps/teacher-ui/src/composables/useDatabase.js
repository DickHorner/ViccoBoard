...
export function useDatabase() {
    ...
    const sportBridge = ref({
        classGroupRepository: {
            ...
            read: async (id) => await db.classGroups.get(id),
            create: async (entity) => {
                const id = createUuid();
                const now = new Date();
                const record = {
        ...
        studentRepository: {
            ...
            read: async (id) => await db.students.get(id),
            findByClassGroup: async (classId) => await db.students.where('classId').equals(classId).toArray(),
            create: async (entity) => {
                const id = createUuid();
                const now = new Date();
                const record = {
        ...
        gradeCategoryRepository: {
            ...
            findByClassGroup: async (classId) => {
        ...
        performanceEntryRepository: {
            ...
            findByStudentAndCategory: async (studentId, categoryId) => {
                const entries = await db.performanceEntries
